...
class Helper:
    ...
    def get_trailer_id(self, soup: BeautifulSoup) -> str:
        try:
            iframe = soup.find("iframe", {"id": "iframe-trailer"})
            data_src = iframe.get("src")
            trailer_id = data_src.strip().strip("/").split("/")[-1]
            return trailer_id
        except:
            return ""
    ...
    def get_title(self, href: str, mvi_content: BeautifulSoup) -> str:
        try:
            heading_name = mvi_content.find("h1").text.strip("\n")
            return heading_name
        except Exception as e:
            self.error_log(
    ...
    def get_description(self, href: str, mvi_content: BeautifulSoup) -> str:
        try:
            description = (
                mvi_content.find("div", class_="desc").text.strip("\n").strip()
            )
            return description
        except Exception as e:
            self.error_log(
    ...
    def get_cover_url(self, href: str, mvi_content: BeautifulSoup) -> str:
        try:
            dm_thumb = mvi_content.find("div", class_="mvic-thumb")
            film_poster_img = dm_thumb.find("img")
            return film_poster_img.get("data-cfsrc", film_poster_img.get("src"))
    ...
    def get_extra_info(self, mvi_content: BeautifulSoup) -> dict:
        extra_info = {}
        try:
            mvic_info = mvi_content.find("div", class_="mvic-info")
            p_elements = mvic_info.find_all("p")
            for p in p_elements:
                key = p.find("strong").text
                value = p.text.replace(key, "")
                value = value.replace("\n", "")
